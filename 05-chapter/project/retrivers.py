...
from utils import clean_text, DocIndexer, IndexerOperator
from config import *
...
class MyRetriever:
    ...
    def find_overlaps(self, doc: List[Document]):
        ...
        for item in doc:
            intervals.append(
                (
                    item.metadata["large_chunks_index_lower_bound"],
                    item.metadata["large_chunks_index_upper_bound"],
                )
            )
    ...
    def get_filter(self, top_k: int, file_md5: str, doc: List[Document]):
        ...
        for chunk_idx in overlaps_k:
            search_dict_docindexer["OR"].append(
                {
                    "large_chunks_index_lower_bound": (
                        IndexerOperator.LTE,
                        chunk_idx,
                    ),
                    "large_chunks_index_upper_bound": (
                        IndexerOperator.GTE,
                        chunk_idx,
                    ),
            ...
            if len(overlaps_k) == 1:
                search_dict_chroma = {
                    "$and": [
                        {"large_chunks_index_lower_bound": {"$lte": overlaps_k[0]}},
                        {"large_chunks_index_upper_bound": {"$gte": overlaps_k[0]}},
                ...
                search_dict_chroma["$or"].append(
                    {
                        "$and": [
                            {"large_chunks_index_lower_bound": {"$lte": chunk_idx}},
                            {"large_chunks_index_upper_bound": {"$gte": chunk_idx}},
                            {"source_md5": {"$eq": file_md5}},
                        ]
                    }
                )
    ...
    def get_relevant_doc_ids(self, docs: List[Document], query: str):
        ...
        id_chain = LLMChain(
            llm=self.llm,
            prompt=PromptTemplates().get_docs_selection_template(),
            output_key="IDs",
        )
